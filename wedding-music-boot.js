(function () {
  ...
  function isEventNavigationHref(href) {
    if (!href) return false;
    return /(?:^|\/)(haldi|nikah|walima)\/?(?:$|[?#])/i.test(href);
  }
  ...
  function musicSrcMatches(src) {
    ...
    try {
      ...
      return bgMusic.src.indexOf("new-audio.mp3") !== -1;
    }
  }
  ...
  function attachEventNavigationHandlers(root) {
    ...
    scope.querySelectorAll('a.event-button[data-event="haldi"], a.event-button[data-event="nikah"], a.event-button[data-event="valima"]').forEach(bindNavigationLink);

    scope.querySelectorAll("a[href]").forEach(function (link) {
      if (link.getAttribute("target") === "_blank") return;
      if (!isEventNavigationHref(link.getAttribute("href"))) return;
